...
if [ -z "${VAULT_CONSUL_TOKEN:-$(awk -F\" '/token/{print $2}' /etc/vault/config.hcl)}" ]; then
	...
	VAULT_CONSUL_TOKEN=$(consul-cli --token="$(awk -F\" '/acl_master_token/{print $4}' /consul/config/consul.json)" --consul="$CONSUL_HTTP_ADDR" acl create --name="$HOSTNAME Vault Token" --rule="key:${VAULT_PATH:-vault}:write" --rule="service:${VAULT_SERVICE_NAME:-vault}:write")
fi
