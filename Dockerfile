...
RUN	echo -n -e "\e[0;32m- Install Containerpilot\e[0m" &&\
	...
	echo -e "#!/bin/sh\nsu-exec consul curl -sS --unix-socket /data/consul.http.sock http://consul/v1/status/leader|jq -re 'if . != \"\" then true else false end'>/dev/null"> /usr/local/bin/consul-healthcheck &&\
